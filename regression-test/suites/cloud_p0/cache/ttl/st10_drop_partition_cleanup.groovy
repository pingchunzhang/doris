        ...
        setFeConfigTemporary(customFeConfig) {
            ...
            try {
                ...
                if (e.getMessage()?.contains("Table [file_cache_info] does not exist")) {
                    ...
                    logger.warn("information_schema.file_cache_info is unavailable, skip file cache assertions in st10_drop_partition_cleanup")
                } else {
                    throw e
                }
            }
            ...
            def getTabletCacheRows = { List<Long> tabletIds ->
                if (!fileCacheInfoAvailable || tabletIds.isEmpty()) {
                    return []
                }
            ...
            long cacheEntryCountBeforeDropPartition = 0L
            long p2CacheEntryCountBeforeDropPartition = 0L
            if (fileCacheInfoAvailable) {
                cacheEntryCountBeforeDropPartition = getTabletCacheEntryCount.call((p1Tablets + p2Tablets).unique())
                p2CacheEntryCountBeforeDropPartition = getTabletCacheEntryCount.call(p2Tablets)
